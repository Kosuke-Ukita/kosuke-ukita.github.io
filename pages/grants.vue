        ...
        <li v-for="(item, i) in group.items" :key="i" class="relative flex gap-1 text-sm">
          ...
          >
            <div class="min-w-max pr-12">
              <p class="whitespace-nowrap"><a :href="item.url" target="_blank" rel="noopener" class="font-medium text-gray-800 dark:text-zinc-200 dark:hover:text-primary hover:text-primary transition-colors">{{ item.name }}</a></p>
