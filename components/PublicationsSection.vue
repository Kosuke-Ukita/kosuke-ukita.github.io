...
const displayPubs = computed(() =>
  props.previewCount !== undefined && props.previewCount > 0
    ? publications.slice(0, props.previewCount)
    : publications
)
const hasMore = computed(() =>
  props.previewCount !== undefined && props.previewCount > 0 && publications.length > props.previewCount
)
const showHover = computed(() => props.previewCount !== undefined)
const groupedPubs = useGroupedPublications()
</script>

<template>
  <!-- Year-grouped view for standalone page -->
  <template v-if="grouped">
    <div v-for="group in groupedPubs" :key="group.year" class="mb-12">
            ...
            <div class="min-w-max pr-12">
              ...
              <div class="flex flex-nowrap items-center gap-2 mt-2">
                ...
                <span v-if="paper.type" class="font-mono text-[0.65rem] border px-1.5 py-0.5 rounded-full shrink-0"
                  :class="paper.type.includes('Spotlight') || paper.type.includes('Oral') || paper.type.includes('Journal') ? 'text-primary border-primary/40' : paper.type.includes('Poster') ? 'text-gray-600 dark:text-zinc-400 border-gray-400 dark:border-zinc-500' : 'text-gray-400 dark:text-zinc-600 border-gray-200 dark:border-zinc-700'"
                >{{ paper.type }}</span>
                <a v-for="link in paper.links" :key="link.name"
                  :href="link.url" target="_blank" rel="noopener" :title="link.name"
                ...
                <button v-if="paper.cite" title="BibTeX"
      ...
      <Transition name="fade">
        ...
        >
          <div class="w-full max-w-xl bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded shadow-2xl">
            ...
            <pre class="bibtex-pre font-mono text-[0.7rem] text-gray-600 dark:text-zinc-400 p-4 overflow-auto max-h-72 whitespace-pre leading-relaxed [scrollbar-width:none]" @wheel="baseHandleWheel">{{ cite.trim() }}</pre>
          </div>
        </div>
      </Transition>
    </Teleport>
      ...
      >
        ...
        >
          <div :class="nowrap ? 'min-w-max pr-12' : ''">
            <p class="pub-title text-sm leading-snug" :class="{ 'whitespace-nowrap': nowrap }">
              <a v-if="paper.links?.find(l => l.name === 'Page')"
                :href="paper.links.find(l => l.name === 'Page')!.url" target="_blank" rel="noopener"
                class="hover:text-primary transition-colors"
              >{{ paper.title }}</a>
            ...
            <p class="pub-authors" :class="{ 'whitespace-nowrap': nowrap }" v-html="highlightAuthor(paper.authors)" />
            <p class="pub-venue" :class="{ 'whitespace-nowrap': nowrap }">{{ paper.venue }}</p>
            <div :class="{ 'whitespace-nowrap': nowrap }">
              <span v-if="paper.date" class="text-xs text-gray-400 dark:text-zinc-500">{{ paper.date }}</span>
              <span v-if="paper.location" class="text-xs text-gray-400 dark:text-zinc-500"> &middot; {{ paper.location }}</span>
            </div>
